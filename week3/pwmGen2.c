...
main (void)
{
    ...
    while (1)
    {
        ...
        if ((checkButton (UP) == PUSHED) && (ui32Freq < PWM_RATE_MAX_HZ))
        {
            ...
            changePWMFreq(&ui32Freq, PWM_RATE_STEP_HZ);
        ...
        if((checkButton(RIGHT) == PUSHED) && (ui32DutyCycle < DUTY_CYCLE_MAX))
        {
            ...
            changeDutyCycle(&ui32DutyCycle, DUTY_CYCLE_CHANGE_AMOUNT);
            setPWM(ui32Freq, ui32DutyCycle);
        }
        //left -- minimum of 5%
...
void changePMWFreq(uint32_t *frequency, uint32_t changeAmt) {
    *frequency += changeAmt;
}

void changeDutyCycle(uint32_t *dutyCycle, uint32_t changeAmt) {
    *dutyCycle += changeAmt;
}
